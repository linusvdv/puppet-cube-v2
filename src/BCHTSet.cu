...
__device__ bool DBCHTSetContains(const PackedState* d_tablebase, const size_t& d_tablebase_size, const DState& key) {
    ...
    for (int j = 0; j < kBucketSize; j++) {
        PackedState tb_data = d_tablebase[(h_1*kBucketSize) + j];
        if (IsSameDState(key, tb_data)) {
            return true;
        }
    ...
    for (int j = 0; j < kBucketSize; j++) {
        PackedState tb_data = d_tablebase[(h_2*kBucketSize) + j];
        if (IsSameDState(key, tb_data)) {
            return true;
        }
        if (tb_data.hash_1 == uint16_t(-1) && tb_data.hash_2 == uint32_t(-1) && tb_data.hash_3 == uint32_t(-1)) {
