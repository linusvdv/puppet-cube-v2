...
bool TranspositionTable::InsertCube(const State& state, const uint8_t& depth) {
    ...
    uint64_t tt_key = SplitMix64((uint64_t(state.hash_2)<<32) ^ uint64_t(state.hash_3));
    tt_key ^= SplitMix64(uint64_t(state.hash_1));
    uint64_t tt_check_hash = (uint64_t(state.hash_1)<<(64-8-16)) | (tt_key/tt_size);
    tt_key %= tt_size;
